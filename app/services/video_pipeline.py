...
def process_video(video_path: str, stride=10, iou_threshold=0.5,
    ...
    for idx, img_orig in enumerate(frames_to_process):
        ...
        # Draw boxes
        #for det in detections:
            # det: tensor [6] -> x1,y1,x2,y2,conf,cls
            #x1f, y1f, x2f, y2f, conf, cls = det.cpu().numpy()
            #x1, y1, x2, y2 = map(int, [x1f, y1f, x2f, y2f])

            #cv2.rectangle(img_rgb, (x1, y1), (x2, y2), (0, 255, 0), 2)
            #cv2.putText(img_rgb, f"{int(cls)} {conf:.2f}", (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        #output_filename = out_dir / f"processed_frame_{idx}.jpg"
        #cv2.imwrite(str(output_filename), cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR))
    ...
    for detections, img_orig in zip(final_detections, frames_to_process):
        ...
        for det in detections:
            ...
            #output_path = out_dir / "digits" / f"digit_{digit_class_name}_{hash(digit_crop.tobytes())}.jpg"
            #cv2.imwrite(str(output_path), cv2.cvtColor(digit_crop_rgb, cv2.COLOR_RGB2BGR))
